document.addEventListener('DOMContentLoaded', () => {
    ...
    if (playButton && promoVideo) {
        playButton.addEventListener('click', () => {
            // Play the video and hide overlay button
            promoVideo.play().then(() => {
                if (videoOverlay) videoOverlay.style.display = 'none';
            }).catch(() => {
                // autoplay might be blocked; keep overlay visible in that case
            });
        });

        // Allow tapping the video to toggle play/pause and overlay
